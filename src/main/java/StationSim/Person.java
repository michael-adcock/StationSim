...
public class Person extends Agent {
    ...
    public Person(int size, Double2D location, String name, Station station, Exit exit, double exitProb) {
        ...
        currentSpeed = 0.0;
        assert exitProb >= 0.0 && exitProb <= 1.0;
        if (station.random.nextDouble() < exitProb) {
            this.exit = exit;
        } else {
            Bag otherExits = station.getExits();
            otherExits.remove(exit);
            this.exit = (Exit) otherExits.get(station.random.nextInt(otherExits.size()));
        }
    }
